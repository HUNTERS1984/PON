...
docker run -d --name elasticsearch --restart=always -ti -p 9200:9200 -v $(pwd)/vagrant/data/elasticsearch:/usr/share/elasticsearch/data:rw,z  --user 1000:50 elasticsearch:1.7.5 elasticsearch
...
docker run -d --name nginx --restart=always -ti -p 80:80 --link mysql:mysql --link phpfpm:phpfpm --volumes-from='app' -e DOMAIN=pon.dev,www.pon.dev -e ENVPON=app_dev  pon/nginx bash
...
yes | sudo  cp -rf $(pwd)/vagrant/bin/queue.sh /usr/bin/queue && sudo chmod a+x /usr/bin/queue